...
func (tq *TripQuery) QueryDailyTrip() *DailyTripQuery {
	query := (&DailyTripClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TripQuery) QueryDailyItinerary() *DailyItineraryQuery {
	query := (&DailyItineraryClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TripQuery) Clone() *TripQuery {
	...
	return &TripQuery{
		...
		withDailyTrip:      tq.withDailyTrip.Clone(),
		withDailyItinerary: tq.withDailyItinerary.Clone(),
...
func (tq *TripQuery) WithDailyTrip(opts ...func(*DailyTripQuery)) *TripQuery {
	query := (&DailyTripClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withDailyTrip = query
	return tq
}
...
func (tq *TripQuery) WithDailyItinerary(opts ...func(*DailyItineraryQuery)) *TripQuery {
	query := (&DailyItineraryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withDailyItinerary = query
	return tq
}
...
func (tq *TripQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Trip, error) {
	var (
		...
		loadedTypes = [3]bool{
			tq.withUser != nil,
			tq.withDailyTrip != nil,
			tq.withDailyItinerary != nil,
		}
	)
	...
	if query := tq.withDailyTrip; query != nil {
		if err := tq.loadDailyTrip(ctx, query, nodes,
			func(n *Trip) { n.Edges.DailyTrip = []*DailyTrip{} },
			func(n *Trip, e *DailyTrip) { n.Edges.DailyTrip = append(n.Edges.DailyTrip, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withDailyItinerary; query != nil {
		if err := tq.loadDailyItinerary(ctx, query, nodes,
			func(n *Trip) { n.Edges.DailyItinerary = []*DailyItinerary{} },
			func(n *Trip, e *DailyItinerary) { n.Edges.DailyItinerary = append(n.Edges.DailyItinerary, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TripQuery) loadDailyTrip(ctx context.Context, query *DailyTripQuery, nodes []*Trip, init func(*Trip), assign func(*Trip, *DailyTrip)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(dailytrip.FieldTripID)
	}
	query.Where(predicate.DailyTrip(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(trip.DailyTripColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TripQuery) loadDailyItinerary(ctx context.Context, query *DailyItineraryQuery, nodes []*Trip, init func(*Trip), assign func(*Trip, *DailyItinerary)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Trip)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(dailyitinerary.FieldTripID)
	}
	query.Where(predicate.DailyItinerary(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(trip.DailyItineraryColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
