...
func (cq *CountryQuery) QueryPoi() *PointsOfInterestQuery {
...
func (cq *CountryQuery) WithPoi(opts ...func(*PointsOfInterestQuery)) *CountryQuery {
	query := (&PointsOfInterestClient{config: cq.config}).Query()
...
func (cq *CountryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Country, error) {
	...
	if query := cq.withPoi; query != nil {
		if err := cq.loadPoi(ctx, query, nodes,
			func(n *Country) { n.Edges.Poi = []*PointsOfInterest{} },
			func(n *Country, e *PointsOfInterest) { n.Edges.Poi = append(n.Edges.Poi, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CountryQuery) loadPoi(ctx context.Context, query *PointsOfInterestQuery, nodes []*Country, init func(*Country), assign func(*Country, *PointsOfInterest)) error {
	...
	query.Where(predicate.PointsOfInterest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(country.PoiColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.country_poi
		if fk == nil {
			return fmt.Errorf(`foreign-key "country_poi" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "country_poi" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
