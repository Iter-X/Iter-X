...
var (
	requireTokenMap = map[string]bool{} // ReadOnly
)
...
func LoadMethodOptions(server *grpc.Server) {
	...
	for _, sd := range sds {
		for _, md := range sd.GetMethods() {
			...
			ext := proto.GetExtension(md.GetMethodOptions(), cnst.E_RequireToken)
			if ext == true {
				requireTokenMap[methodName] = true
			}
		}
	}
}
...
func TokenValidation(validator TokenValidator) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
		if requireTokenMap[info.FullMethod] {
