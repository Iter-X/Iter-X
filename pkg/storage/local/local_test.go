...
func Test_NewLocalOSS(t *testing.T) {
	...
	mux.HandlePath(c.GetUploadMethod(), c.GetUploadURL(), func(w http.ResponseWriter, r *http.Request, pathParams map[string]string) {
		localOSS.UploadHandler(w, r)
	})

	mux.HandlePath("GET", c.GetPreviewURL(), func(w http.ResponseWriter, r *http.Request, pathParams map[string]string) {
		localOSS.PreviewHandler(w, r)
	})
